      ...
      container.innerHTML = filtered.map(b => `
        <div class="ri" onclick="BelegeModule.openDetail(${b.id})">
          ...
          <div class="ri-inf">
            ...
            <div class="ri-me">${(b.cat||'').includes('Bewirtung') ? '<span class="badge" style="background:var(--orn);color:#fff;padding:2px 6px;border-radius:4px;font-size:9px;margin-right:4px">Bewirtung 70/30</span>' : ''}<span>${BSP.fd(b.date)}</span>${b.type === 'ar' ? '' : ` · <span>${b.cat || '—'}</span>`}</div>
          </div>
          <div class="ri-r">
  ...
  async function openDetail(id) {
    ...
    const html = `
      ...
      <div class="mod-header" style="text-align:center">
        ...
        <p class="mod-sub">${BSP.fd(b.date)}${b.type === 'ar' ? (b.belegNr ? ' · ' + b.belegNr : '') : ' · ' + (b.belegNr || b.cat || 'Allgemein')}</p>
      </div>
